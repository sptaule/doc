            ...
            <div class="w-full flex flex-col items-start justify-center space-y-3">
                ...
                @foreach(\App\Models\Navigation::getMenus() as $menu)
                    <div class="flex items-center justify-start">
                        ...
                        <p class="text-gray-700 rounded-md py-2 px-3 inline-flex items-center text-sm font-medium">
                            <span class="mr-1 text-gray-500 font-arima text-base">{{ $menu->name }}</span>
                        ...
                        <div>
                            @foreach(\App\Models\Navigation::getPages("m.id = $menu->id") as $childPage)
                                <a href="/{{ $menu->slug }}/{{ $childPage->slug }}"
                                   class="text-gray-700 hover:bg-scuba-green hover:text-white rounded-md py-2 px-1 inline-flex items-center text-sm font-medium">
                                    {{ $childPage->name }}
                                </a>
                            @endforeach
                        </div>
